make_dPsiPlot <- function(dpsi_df, pdir, num_thresh = 30) {
  ...
  dpsi_df$hgnc <- unlist(lapply(dpsi_df$gene, function(x) {ifelse(unlist(lapply(strsplit(x, split = "[.]"), "[[", 1)) %in% hg38.conv$ens,
                                                                  unique(hg38.conv$gene_name[hg38.conv$gene_id == unlist(lapply(strsplit(x, split = "[.]"), "[[", 1))]), x)
